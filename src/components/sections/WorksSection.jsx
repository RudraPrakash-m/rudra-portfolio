...
export const WorksSection = ({ sectionRef, isActive = true }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Section Heading */}
        <h2 className="work-row text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-[#1c1c1c] mb-1 sm:mb-2">
          Works
        </h2>
        ...
        {rudraData.projects.map((item, idx) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="work-row group flex items-center justify-between w-full py-1 sm:py-1.5 cursor-pointer text-[#1c1c1c] hover:opacity-75 transition-opacity"
          >
            <div className="flex items-center gap-2.5 sm:gap-4">
              ...
              <div className="text-left">
                <span className="font-medium tracking-tight text-base sm:text-2xl md:text-3xl lg:text-4xl leading-tight block">
                  {item.title}
                </span>
                <span className="text-[10px] sm:text-xs md:text-sm text-[#1c1c1c]/60 font-mono mt-0.5 block">
                  {item.category}
                </span>
              </div>
            </div>

            <div className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 text-neutral-600 group-hover:text-black shrink-0">
